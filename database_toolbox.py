...
def Database_Files():
	...
	choice = Dialogue.yesno(Addon_Title, '[COLOR %s]Database Files: [LIGHT](Blank Row)[CR][COLOR %s] > Add Blank row between each new line.[CR] > No Blank row between each new line.[/LIGHT][/COLOR][CR]Add a blank row between each new line ?[/COLOR]' % (TEXT_GENERAL, TEXT_ITEM), yeslabel = ('[COLOR %s]Add Blank[/COLOR]' % TEXT_VALUE), nolabel = ('[COLOR %s]No Blank[/COLOR]' % TEXT_HIGHLIGHT))
	...
	database = "\n\n".join(database_paths) if BLANK_ROW == 'true' else "\n".join(database_paths)
